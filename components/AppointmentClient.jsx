...
import { addAppointment, deleteAppointment, updateAppointment } from "@/lib/actions";
...
const AppointmentClient = ({ initialEvents, userId }) => {
  ...
  const [alarmSound] = useState(typeof Audio !== "undefined" ? new Audio("/audio.mp3") : null);
  ...
  function handleDateClick(arg) {
    ...
    const formattedDate = new Date(arg.date.getTime() - (arg.date.getTimezoneOffset() * 60000))
      .toISOString()
      .split("T")[0];
    const formattedTime = new Date(arg.date.getTime() - (arg.date.getTimezoneOffset() * 60000))
      .toISOString()
      .split("T")[1]
      .slice(0, 5);
    ...
    setNewEvent({
      ...newEvent,
      start: formattedDate,
      notificationTime: `${formattedDate}T${formattedTime}`,
      allDay: arg.allDay,
      id: new Date().getTime(),
      userId: userId,
    });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (isEditing) {
        await updateAppointment(eventData);
        setAllEvents(allEvents.map(event => event.id === newEvent.id ? eventData : event));
      ...
      console.error("Failed to add/update appointment:", err);
  ...
  const handleNotificationTimeChange = (e) => {
    setNewEvent({
      ...newEvent,
      notificationTime: `${newEvent.start}T${e.target.value}`,
    });
  };
                ...
                >
                  <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      <input
  type="time"
                          value={newEvent.notificationTime ? newEvent.notificationTime.split("T")[1] : ""}
...
/>
        ...
        <Transition.Root show={showOptionsModal} as={Fragment}>
          <Dialog as="div" className="relative z-10" onClose={setShowOptionsModal}>
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0"
                    ...
                    <div>
                      <div className="text-center sm:mt-5">
                        ...
                        <div className="mt-2 flex gap-4">
                          <button
                            onClick={handleEdit}
                            className="mt-3 inline-flex w-full justify-center rounded-md border border-transparent bg-violetdesc px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-blue-900 sm:col-start-1 sm:mt-0 sm:text-sm"
                          >
                            <Edit2
                        size="22"
                        color="currentColor"
                      ...
                      />
                          ...
                          <button
                            ...
                            className="mt-3 inline-flex w-full justify-center rounded-md border border-transparent bg-red-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 sm:col-start-1 sm:mt-0 sm:text-sm"
                          >
                             <Trash size="22" color="currentColor" className="mr-2 dark:text-white" />
                            Delete
                          </button>
                        </div>
                      </div>
...
//   //   // Format the date to ensure the correct date is used
//   //   const formattedDate = new Date(arg.date.getTime() - (arg.date.getTimezoneOffset() * 60000))
//   //     .toISOString()
//   //     .split("T")[0];
...
//     // Format the date and time to ensure the correct values are used
//     const formattedDate = new Date(arg.date.getTime() - (arg.date.getTimezoneOffset() * 60000))
//       .toISOString()
//       .split("T")[0];
//     const formattedTime = new Date(arg.date.getTime() - (arg.date.getTimezoneOffset() * 60000))
//       .toISOString()
//       .split("T")[1]
//       .slice(0, 5); // Extract only the time part (HH:MM)
...
//                         <input
//                           type="time"
//                           value={newEvent.notificationTime.split("T")[1] || ""}
//                           onChange={handleNotificationTimeChange}
//                           className="border-2 rounded-md p-2 w-full"
//                           required
//                         />
