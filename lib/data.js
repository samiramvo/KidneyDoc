...
import { User, Patient, Consultation, Appointment, Note , Document} from "@/lib/models";
...
export const fetchNoteById = async (noteId) => {
  try {
    ...
    if (note) {
      ...
      const patient = await Patient.findById(note.patientId).lean();
      ...
      if (patient) {
        ...
        console.log("Patient non trouvé avec l'ID :", note.patientId);
      }
    }
...
export const fetchDocumentById = async (documentId) => {
  try {
    ...
    if (document) {
      ...
      const patient = await Patient.findById(document.patientId).lean();
      ...
      if (patient) {
        ...
        console.log("Patient non trouvé avec l'ID :", document.patientId);
      }
    }

    console.log("Document récupérée :", document);
    return document;
  } catch (err) {
    console.error(err);
